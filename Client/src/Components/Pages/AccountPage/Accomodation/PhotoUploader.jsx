  ...
  return (
    <>
      <div className="p-1 flex flex-col border border-1 border-gray-200 focus:outline-1 rounded-lg px-2 text-sm text-slate-500 space-y-2">
        <label className="lg:text-base md:text-base sm:text-sm text-xs mb-2">
          Upload Images
        </label>
        ...
        <div className=" grid lg:grid-cols-4 md:grid-cols-4 sm:grid-cols-4 grid-cols-2 gap-1 ">
          {addedPhotos.length > 0 &&
            addedPhotos.map((link) => (
              <div className="flex relative" key={link}>
                ...
                >
                  {link === addedPhotos[0] && (
                    <i className="fa-solid fa-star text-white cursor-pointer bg-black bg-opacity-70 p-0.5 rounded-sm"></i>
                  )}
                  {link !== addedPhotos[0] && (
                    <i className="fa-regular fa-star  text-white cursor-pointer bg-black bg-opacity-70 p-0.5 rounded-sm"></i>
                  )}
                </button>
                <button
